...
*** Open and Closed
The =Data.Functor.Polynomial.Variant= module provides two data types, =VariantF= and =ClosedVariantF=.
The former is constructed using =inj=, much like in =purescript-variant=, and for the latter, it takes some
=VariantF= whose fields have =Functor=, =Bifunctor=, and =Dissect= instances. The dichotomy between these
two types allow for an interesting pattern to arise: extensible data types:
#+begin_src purescript
type Scoops = Int
