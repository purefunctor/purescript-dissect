...
match against. This package provides the ~ClosedVariantF~ dissectible which allows one to use a variant type
...
*** Deferred Instances
The =Data.Functor.Polynomial.Variant= module provides two data types, =PreVariantF= and =VariantF=.
The former is constructed using =inj=, much like in =purescript-variant=, and for the latter, it takes some
=PreVariantF= whose fields have =Functor=, =Bifunctor=, and =Dissect= instances. By deferring when these
instances are required, we're able to create extensible data types with relative ease.
#+begin_src purescript
type Scoops = Int
